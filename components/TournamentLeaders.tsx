...
function LeaderSection({ title, rows, unit }: { title: string; rows: LeaderRow[]; unit: string }) {
      ...
      {rows.map((row, i) => (
        <Link key={row.id} href={row.href}>
          <div className="flex items-center gap-3 px-[18px] py-2.5 border-b border-pitch-rule hover:bg-pitch-cream transition-colors cursor-pointer">
            <span className="font-sans text-[11px] text-pitch-ink-light w-4 flex-shrink-0">{i + 1}</span>
            <div className="flex-shrink-0 w-[26px] h-[26px] rounded-full bg-pitch-green-light flex items-center justify-center">
              <span className="font-sans text-[9px] font-medium text-pitch-green">{getInitials(row.name)}</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-sans text-[12px] font-medium text-pitch-ink leading-tight">{row.name}</p>
              <p className="font-sans text-[10px] text-pitch-ink-light">{row.team}</p>
            </div>
            <span className="font-sans text-[16px] font-medium text-pitch-ink flex-shrink-0">{row.count}</span>
          </div>
        </Link>
      ))}
    </div>
...
export default function TournamentLeaders({ topPlayers, completedMatches }: TournamentLeadersProps) {
  ...
  return (
    <div>
      <LeaderSection title="Top scorers" rows={scorers} unit="goals" />
      <LeaderSection title="Top assists" rows={assisters} unit="assists" />
      <LeaderSection title="Clean sheets" rows={cleanSheets} unit="clean sheets" />
    </div>
  );
}
